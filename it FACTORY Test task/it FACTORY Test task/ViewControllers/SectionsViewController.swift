...
class SectionsViewController: UIViewController {
    ...
    var dataSource: UICollectionViewDiffableDataSource<Sections, Section>?
    ...
    private func reloadData() {
        var snapshot = NSDiffableDataSourceSnapshot<Sections, Section>()
        
        snapshot.appendSections([.eatingHabits, .discipline, .eatingScience, .eatingWell])
        snapshot.appendItems(sections ?? [], toSection: .eatingHabits)
        snapshot.appendItems(sections ?? [], toSection: .discipline)
        snapshot.appendItems(sections ?? [], toSection: .eatingWell)
        snapshot.appendItems(sections ?? [], toSection: .eatingScience)
...
extension SectionsViewController {
    private func ceateDataSource() {
        dataSource = UICollectionViewDiffableDataSource<Sections, Section>(collectionView: collectionView, cellProvider: { collectionView, indexPath, itemIdentifier in
            ...
            case .eatingHabits,
                ...
                let section = sections?[indexPath.section].items[indexPath.item]
                
             
                    cell.configurationCell(image: section.image.the1X, title: section.title)
        ...
        dataSource?.supplementaryViewProvider = {
            ...
            if let sectionHeader = sectionHeader {
                sectionHeader.configurationCell(text: self.sections?[indexPath.section].header ?? "")
            }
